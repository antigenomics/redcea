#!/bin/bash
#SBATCH --job-name=tcrempnet_leiden_grid_tra_top8
...
LOGDIR="/projects/immunestatus/vdjdb/tcrempnet_logs_tra"
mkdir -p "${LOGDIR}"
...
# Leiden grid (expanded)
kns = [2, 4, 8, 16, 25, 40, 64, 80, 100, 150, 200]
lrs = [0.5, 1.0, 1.5, 2.0, 2.5, 3.0, 4.0, 5.0, 7.5, 10.0]
configs = [(kn, lr) for kn in kns for lr in lrs]  # 110

n_ep = len(epitopes)
n_cfg = len(configs)
total = n_ep * n_cfg  # should be 336
