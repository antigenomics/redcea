...
def compute_embeddings_if_needed(
    ...
    *,
    ...
    rep = load_analysis_repertoire(
        ...
        lib,
        locus,
        mapping_column=config.index_col,
        llen=config.lower_len_cdr3,
        hlen=config.higher_len_cdr3,
    )
    ...
    if not hasattr(proto, "total"):
        ...
        proto = load_prototype_repertoire(proto_path, lib, locus, mapping_column=config.index_col)
